...
export const NotifySubscribers = (call: grpc.ServerUnaryCall<NotifySubscribersRequest, NotifySubscribersResponse>, callback: grpc.sendUnaryData<NotifySubscribersResponse>) => {
      ...
      connection.query('SELECT * FROM subscriptions', (err, result) => {
        ...
        if (err) {
          ...
          for (let i = 0; i < length; ++i) {
            console.log(`${type === NotificationType.NOTIFICATION_TASK_COMPLETED
              ? "Completed"
              : "Missed Deadline"} notification for "${result[i].firstname} | sent to ${result[i].email}`);
          };
